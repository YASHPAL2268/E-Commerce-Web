...
// ✅ Add product to cart (if exists → increase qty, else push new product)
router.post("/add", async (req, res) => {
    const { userId, productId } = req.body;
...
// ✅ Fetch cart items for a specific user (with product details populated)
router.get("/:userId", async (req, res) => {
    const cart = await Cart.findOne({ userId: req.params.userId }).populate("items.productId");
...
// ✅ Update product quantity inside a user's cart
router.put("/:userId/update/:productId", async (req, res) => {
    const { userId, productId } = req.params;
    const { quantity } = req.body;
